...
internal static unsafe class LinuxNative
{
    ...
    internal static StatWitness StatxFd(int fd){uint req=STATX_BASIC_STATS|STATX_BTIME|STATX_MNT_ID|STATX_MNT_ID_UNIQUE;if(statx(fd,"",AT_EMPTY_PATH,req,out var st)!=0){int e=Errno;if(e is 2 or 116)throw new ProviderException("destroyed","Retained file descriptor no longer resolves in the provider.",e);throw new ProviderException("native_error","statx(fd) failed.",e);}return FromStatx(st);}
    ...
    {
        byte[] bytes=Convert.FromBase64String(handle.BytesBase64);IntPtr p=Marshal.AllocHGlobal(8+bytes.Length);try{Marshal.WriteInt32(p,bytes.Length);Marshal.WriteInt32(p,4,handle.Type);Marshal.Copy(bytes,0,IntPtr.Add(p,8),bytes.Length);lastErrno=0;string dev=$"{devMajor}:{devMinor}";foreach(var mount in Mounts().Where(m=>m.MajorMinor==dev)){int mfd=open(mount.MountPoint,O_RDONLY|O_CLOEXEC|O_DIRECTORY);if(mfd<0){lastErrno=Errno;continue;}try{int fd=open_by_handle_at(mfd,p,(write?O_RDWR:O_RDONLY)|O_CLOEXEC);if(fd>=0)return fd;lastErrno=Errno;}finally{close(mfd);}}return -1;}finally{Marshal.FreeHGlobal(p);}
    }
