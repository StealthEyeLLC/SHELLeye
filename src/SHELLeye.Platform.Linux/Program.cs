...
internal static class Program
{
    ...
    private static object ProcessStart(JsonElement p)
    {
        ...
        int pid=proc.Id;LinuxNative.ProcInfo info;try{using var h=LinuxNative.OpenVerifiedPidfd(pid,null,true);info=h.Process;}catch{try{if(!proc.HasExited)proc.Kill(true);}catch{}try{proc.WaitForExit(1000);}catch{}proc.Dispose();throw;}Spawned[pid]=proc;_=DrainAndCleanupAsync(proc,pid);return ProcessResult(info);
    }
    private static async Task DrainAndCleanupAsync(Process p,int pid){try{Task a=p.StandardOutput.BaseStream.CopyToAsync(Stream.Null),b=p.StandardError.BaseStream.CopyToAsync(Stream.Null);await p.WaitForExitAsync();await Task.WhenAll(a,b);}catch{}finally{Spawned.TryRemove(pid,out _);p.Dispose();}}
