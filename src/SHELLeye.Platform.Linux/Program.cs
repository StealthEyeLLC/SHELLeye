...
internal static class Program
{
    ...
    {
        CheckWorld(p);string executable=RequiredString(p,"executable");var psi=new ProcessStartInfo(executable){UseShellExecute=false,RedirectStandardOutput=true,RedirectStandardError=true,WorkingDirectory=OptString(p,"cwd")??Environment.CurrentDirectory};
        if(p.TryGetProperty("args",out var ax)&&ax.ValueKind==JsonValueKind.Array)foreach(var a in ax.EnumerateArray())psi.ArgumentList.Add(a.GetString()??"");
        if(p.TryGetProperty("environment",out var env)&&env.ValueKind==JsonValueKind.Object)foreach(var kv in env.EnumerateObject())psi.Environment[kv.Name]=kv.Value.GetString()??"";
        Process proc;try{proc=Process.Start(psi)??throw new ProviderException("native_error","Process.Start returned null.");}catch(System.ComponentModel.Win32Exception e)when(e.NativeErrorCode==2){throw new ProviderException("not_found","Executable not found.",e.NativeErrorCode,inner:e);}
        Spawned[proc.Id]=proc;_=DrainAndCleanupAsync(proc);LinuxNative.ProcInfo? info=null;for(int i=0;i<50;i++){try{using var h=LinuxNative.OpenVerifiedPidfd(proc.Id,null);info=h.Process;break;}catch(ProviderException e)when(e.Code is "destroyed" or "not_found"){Thread.Sleep(10);}}if(info is null)throw new ProviderException("unknown","Started process did not become observable with a pidfd/start-time witness.");return ProcessResult(info);
    }
    private static async Task DrainAndCleanupAsync(Process p){try{Task a=p.StandardOutput.BaseStream.CopyToAsync(Stream.Null),b=p.StandardError.BaseStream.CopyToAsync(Stream.Null);await p.WaitForExitAsync();await Task.WhenAll(a,b);}catch{}finally{Spawned.TryRemove(p.Id,out _);p.Dispose();}}
